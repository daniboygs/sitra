...
import { getVictims, getFiscalias, getQueryList, setQueryBinnacle, getIP} from '../Service';
...
class Victims extends React.Component{
  ...
  handleSetData = (response) => {
    this.setState({
      ...
      victims: this.depurateData(response),
      filteredData: this.state.victims
    });
  }
  ...
  handleProcess(e){
    ...
    if(this.state.selectedYears.length && this.state.selectedFiscalias.length && this.state.selectedCities.length && this.state.selectedCrimes.length && this.state.selectedRequests.length){
      ...
      console.log(request);
      getVictims(request).then((response) => {
  ...
  handleProcessBinnacle(type){
    ...
    let requestFields = [];
    var sexo=[], edad=[], escolaridad=[], profesion=[], nacionalidad=[], arma=[], mes=[], año=[], fiscalia=[], municipio=[], delito=[];

    for(var k in this.state.victims) {
      let victim = this.state.victims[k];
      ...
      if(requestFields.includes("Sexo"))(sexo.push(this.state.victims[k]["Sexo"]))
      if(requestFields.includes("Edad"))(edad.push(this.state.victims[k]["Edad"]))
      if(requestFields.includes("Escolaridad"))(escolaridad.push(this.state.victims[k]["Escolaridad"]))
      if(requestFields.includes("Profesion"))(profesion.push(this.state.victims[k]["Profesion"]))
      if(requestFields.includes("Nacionalidad"))(nacionalidad.push(this.state.victims[k]["Nacionalidad"]))
      if(requestFields.includes("Tipo de arma"))(arma.push(this.state.victims[k]["Tipo de arma"]))
      if(requestFields.includes("Mes"))(mes.push(this.state.victims[k]["Mes"]))
      if(requestFields.includes("Año"))(año.push(this.state.victims[k]["Año"]))
      if(requestFields.includes("Fiscalia"))(fiscalia.push(this.state.victims[k]["Fiscalia"]))
      if(requestFields.includes("Municipio"))(municipio.push(this.state.victims[k]["Municipio"]))
      if(requestFields.includes("Delito"))(delito.push(this.state.victims[k]["Delito"]))
    ...
    getIP().then((response) => {
      ...
      (error)=>{
        console.log(error);
        this.setBinnacle(allRequestValues, filteredAttributes, realValueFilters, notValueFilters, type, "");
      }
    );
  ...
  setBinnacle(allRequestValues, filteredAttributes, realValueFilters, notValueFilters, type, geoplugin){
    ...
    const request = {
      ...
      section: 2, //1 - Folders, 2 - Victims, 3 - Imputeds
      geoplugin: geoplugin
    }
  ...
  handleShowProcessingDataAlert = (event) => {
    ...
    if ((event.target.className == "pvtAxisContainer pvtVertList pvtRows" || event.target.className == "pvtAxisContainer pvtHorizList pvtCols") && this.state.victims.length>20000){
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Query
          showQuery={this.state.victims.length? true: false}
