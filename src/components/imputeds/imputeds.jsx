...
class Imputeds extends React.Component{
  ...
  handleSetData = (response) => {
    this.setState({
      ...
      imputeds: this.depurateData(response),
      filteredData: this.state.imputeds
    });
  }
  ...
  handleProcessBinnacle(type){
    ...
    let requestFields = [];
    var sexo=[], edad=[], escolaridad=[], profesion=[], nacionalidad=[], parentesco=[], arma=[], mes=[], año=[], fiscalia=[], municipio=[], delito=[], detenido=[];
    ...
    for(var k in this.state.imputeds) {
      ...
      if(requestFields.includes("Detenido"))(detenido.push(this.state.imputeds[k]["Detenido"]))
  ...
  setBinnacle(allRequestValues, filteredAttributes, realValueFilters, notValueFilters, type, geoplugin){
    ...
    const request = {
      ...
      section: 2, //1 - Folders, 2 - imputeds, 3 - Imputeds
      geoplugin: geoplugin
    }
  ...
  handleShowProcessingDataAlert = (event) => {
    ...
    if ((event.target.className == "pvtAxisContainer pvtVertList pvtRows" || event.target.className == "pvtAxisContainer pvtHorizList pvtCols") && this.state.imputeds.length>20000){
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Query
          showQuery={this.state.imputeds.length? true: false}
